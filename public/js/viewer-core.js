...
const sockets = {}; // roomId => WebSocket
 ...
 function startViewerDynamic(roomId, containerId) {
  ...
  ws.onopen = () => {
    ...
    const peer = new kurentoUtils.WebRtcPeer.WebRtcPeerRecvonly(options, function (err) {
      ...
      if (parsed.id === 'viewerResponse') {
        ...
        if (onStreamStartedCallbacks[roomId]) {
          try {
            ...
            console.warn(`⚠️ onStreamStartedCallback error for room ${roomId}`, e);
          } finally {
 ...
 function stopViewer(roomId) {
  ...
  if (onStreamEndedCallbacks[roomId]) {
    try {
      ...
      console.warn(`⚠️ onStreamEndedCallback error for room ${roomId}`, e);
    } finally {
...
export function startViewer(roomId, videoElement, onStreamReady) {
  ...
  ws.onopen = () => {
    const options = {
      ...
      onicecandidate: (candidate) => {
        ws.send(JSON.stringify({ id: 'onIceCandidate', room: roomId, candidate }))
      }
    }
